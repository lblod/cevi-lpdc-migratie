...
async function main() {
    ...
    const abbProducts: (AbbProduct | undefined)[] = ceviProducts.map((ceviProduct: CeviProduct): AbbProduct | undefined => {
        try {
    ...
    const triples = abbProducts.flatMap(abbProduct => abbProduct?.toTriples(Language.INFORMAL)).map(trip => trip?.toString()).join('\n');
    ...
    await fs.writeFile('20231016-migration-stekene.ttl', triples);
